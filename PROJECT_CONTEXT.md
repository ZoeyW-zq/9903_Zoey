...
## VR Version Status (2026-08-23)

The current development focus has shifted to the VR version, built in `scene_VR.unity` on top of the completed WebGL flow. `scene_VR_old.unity` is kept as the previous VR iteration.

- VR interaction is XR Interaction Toolkit 3.3.1-based: memory objects carry `XRGrabInteractable` alongside the legacy EZPZ `Holdable`. `MemoryPlacementItem.SetGrabbable(bool)` toggles both components, and `GameStateController.RestoreResolvedMemoryObjects()` uses `SetGrabbable` instead of touching `Holdable` directly.
- `GameStateController`'s `player` reference in scene_VR points to the `XR Origin (XR Rig)` root (also tagged `Player`); the Main Camera sits under Camera Offset as usual.
- Assistant `LookAtRotate` (EZPZ): the recommended target is the head (`Main Camera` under Camera Offset), NOT the XR Origin root. The rig root does not move with head tracking or physical walking, so a rig-root target makes the assistant stare at a fixed floor anchor. The WebGL scene works because its player object moves with the view.
- UI: every interactive canvas needs at least a `TrackedDeviceGraphicRaycaster` so XR rays can press buttons without colliders. The official XRIT 3.3.1 `UI Sample.prefab` keeps the plain `GraphicRaycaster` together with the tracked one and uses Screen Space - Camera, so both of those are fine and should NOT be changed. The EventSystem already has `XRUIInputModule`, and the Near-Far Interactors have `m_EnableUIInteraction: 1`. The real gap: the mirror-dialogue canvases (under DungeonRoot) and the `ModalSingleButton` instances inherited from the WebGL scene have NO raycaster at all and need `TrackedDeviceGraphicRaycaster` added.
- The crystal-ball show/hide bug is fixed (2026-08-23): `CrystalBall.SetEnabled` now activates/deactivates the crystal-ball GameObject like `CrystalBall_WebGL` does.
- Editor Play Mode testing: the installed package is XRIT **3.4.1** (samples on disk are labeled 3.3.1 but the rig is a faithful copy of the official prefab). In-editor testing uses the XR Interaction Simulator (`Assets/Samples/XR Interaction Toolkit/3.4.1/XR Interaction Simulator/`). Pitfall found 2026-08-23: the Main Camera must keep the `MainCamera` tag — the simulator finds the camera through `Camera.main`, and a changed tag silently kills all simulated input (no movement, no rays). Standalone XR Management currently enables only the OpenXR loader. The leftover `Assets/XR/Loaders/SimulationLoader.asset` references a script GUID that no longer exists in the project and is safe to delete.

## Convai SDK Import Status (2026-08-26)

Convai Unity SDK is now installed through the Package Manager at version **4.5.0** (`com.convai.convai-sdk-for-unity`). The Package Manager route requires the Unity account to hold the Asset Store entitlement for the Convai product. The previous embedded 4.4.1 Assets copy has been removed. A separate `Assets/Convai/Plugins` directory remains and should not be removed without validating native plugin ownership. Unity has regenerated the Convai modules, transports, and runtime assemblies under `Library/ScriptAssemblies`; the official Convai Sample scene has been tested successfully by the user.

`Packages/manifest.json` and `Packages/packages-lock.json` contain and resolve `com.convai.convai-sdk-for-unity` at `4.5.0`. Its Newtonsoft dependency is present as `com.unity.nuget.newtonsoft-json` `3.2.2`. `Assets/Resources/ConvaiSettings.asset` exists; credentials are project-local configuration and must not be printed or committed in plain text.

`scene_VR_ConvaiPrototype.unity` now contains `ConvaiManager`, `ConvaiRoomManager`, `ConvaiPlayer`, `ConvaiCharacter` (`Mio`), `ConvaiAudioOutput`, and a synced `ConvaiNarrativeDesignManager`. Fifteen unique backend Sections are synced with no orphaned entries or recorded fetch error. PushToTalk, Connect On Start, and 3D character voice are enabled. `scene_VR.unity` remains on the original assistant implementation.

Unity-to-Narrative integration is now partially implemented for the Office proof: `ConvaiNarrativeBridge` handles Office Section changes, exploration return, Start Work completion, and the Memory Room entry trigger. The prototype uses an opt-in ExternalConvai mode on `OfficeDialogueController`; the existing `AssistantController` remains unchanged for the regular scene. Full Quest runtime behavior, backend action/decision wiring, and later-stage Crisis/Mirror/Report triggers still require Play Mode testing and further bridge work.

The Convai prototype has no `AssistantController` component and cannot play the old recorded Office clips. If the assistant says content resembling the old script, it is generated from the Convai Dashboard character/Narrative content or the Editor is running a stale in-memory scene. Convai Diagnostics are kept at Warning with stack traces disabled because Info-level live transcription logs with full stacks caused heavy Editor stutter.

The Office bridge listens to `ConvaiNarrativeDesignManager.OnAnySectionChanged` in code; Section UnityEvents should remain empty to avoid duplicate actions. Start Work uses Mio's speech start/stop events before showing the PC report. `memory_room_initial_placement` must be reachable only through the Unity-sent `memory_room_entered` trigger after the Hippocampus fade completes, never through a direct Decision from `office_crystal_ball`.

Play Mode can run `Temp/__Backupscenes/0.backup` when the active scene is dirty. That in-memory backup may omit newer disk-side Convai wiring. Reopen `scene_VR_ConvaiPrototype.unity` from disk before validation and confirm the Console logs `ConvaiNarrativeBridge active: Office flow is connected to Narrative Design.`

The Convai Character Background and Sample Dialogues must not contain the full future story sequence. Those global fields apply in every Section and caused Mio to narrate transitions that Unity had not performed. Stage-specific plot information belongs only in the matching Section Objective, with a global rule forbidding unconfirmed transitions and future-section foreshadowing. The bridge polls the current Section as an event fallback. An empty Section ID immediately after Character Ready is valid until the first backend Section transition and must not be treated as an error.

Convai Dashboard audit (2026-08-26, corrected 2026-08-27): Narrative Design is enabled with `office_introduction` as the start Section. The Legacy Playground control is an action toggle: while Narrative is enabled it is labelled `Disable Narrative Design` and appears checked; clicking it changes the backend field from `is_narrative_driven=true` to `false`, after which the control is labelled `Enable Narrative Design` and appears unchecked. A temporary verification toggle was performed and then restored, so the backend currently reports `is_narrative_driven=true`. Mio's global Core Description and Speaking Style were rewritten to remove future-stage information and forbid unconfirmed transitions; Sample Dialogues were shortened; Core AI temperature was reduced from 0.7 to 0.4; Personality Extraversion and Agreeableness were set to 3. `office_report_ready` and `memory_room_entered` names/messages/destinations match the Unity bridge. Restart the Unity Play Mode session after Dashboard changes so the room reconnects with the active Narrative configuration.

Runtime/backend verification (2026-08-26): the Convai REST state reports `is_narrative_driven=True`, `office_introduction` as the start Section, saved Trigger names and destinations matching the Unity bridge, and all Office Decisions pointing to the intended Sections. Earlier Console text saying `trigger sent` was misleading because SDK 4.5.0 returns success when it only queues a Trigger before Character Ready. `ConvaiNarrativeBridge` now holds the initial Office entry locally, waits for `OnCharacterReady`, waits one additional frame for queued template keys to flush, and only then invokes `office_entered`. It reports an explicit error after 45 seconds without readiness, distinguishes queued requests from requests handed to the transport, and listens to full Section Data as well as Section Change events. `ConvaiNarrativeBridgeTests.OfficeEntryWaitsForCharacterReadyBeforeInvokingTrigger` covers the pre-ready guard.

Input-mode verification (2026-08-27): the prototype intentionally uses SDK `PushToTalk` (`_turnTakingOptions.<Mode>k__BackingField: 1`) to avoid incidental speech consuming interactions. PushToTalk is not a Narrative limitation: once Convai emits a processed-final player transcript, the backend should evaluate the active Section Decisions normally.

Start Work reliability (2026-08-27): official Convai documentation confirms that `InvokeTrigger()` returning `true` means only that the request was accepted locally; the graph has advanced only after a backend `behavior-tree-response` produces `NarrativeSectionChanged`. The backend now includes the deterministic saved Trigger `office_start_work_requested -> office_start_work`. `ConvaiNarrativeBridge` listens to `ConvaiManager.Events.OnFinalUserTranscriptionReceived`; when a processed-final transcript matches a restricted start-work intent whitelist, it waits 2.5 seconds for the dashboard Decision and lets any current Mio reply finish, then invokes that Trigger only if `office_start_work` has not arrived. This preserves free-form Decisions while preventing a missed Decision from leaving Unity behind Mio's spoken response.

Narrative bootstrap fix (2026-08-27): Console evidence showed processed-final player transcripts and both saved Triggers reaching the transport while `CurrentSectionId` remained empty and no `behavior-tree-response` arrived. `office_introduction` had been configured both as the backend Start Section and as the destination of `office_entered`, creating a same-section bootstrap transition. The graph now starts in a silent `runtime_bootstrap` Section (`6e211e02-a1c0-11f1-828d-42010a7be02f`); `office_entered` performs the first real transition to `office_introduction`. The new Section is synced into the prototype scene with empty UnityEvents.

Live-session backend audit (2026-08-27): Unity logged valid room and character session IDs while `CurrentSectionId` remained empty. Calling Convai `character/narrative/get-current-section` with both the room session ID and character session ID returned `No narrative interaction found for the given character_id, and session_id` (latest Reference IDs `8de56995-1996-4510-ae82-d6e92a192f68` and `2e97e068-277b-4ec4-889b-dd5d0815c293`). Production REST and realtime environments match, the Character ID matches, and REST still reports Narrative enabled with `runtime_bootstrap` as Start Section. This places the remaining failure in Convai Core session initialization rather than Unity transcript, Trigger dispatch, or Section-event handling.

kiki migration (2026-08-27): the fresh `Narrative Probe` character (`33eac386-a1c6-11f1-a23e-42010a7be02f`) passed a minimal `probe_idle -> probe_active` saved-trigger test and has been renamed `kiki`. Mio's Core Description, Speaking Style, Sample Dialogues, voice, language, Temperature 0.4, Personality values, State Of Mind setting, 15 formal Sections, 11 formal Triggers, and all five Decision groups were migrated. Text references to Mio were replaced with kiki. The validated `gpt-4o-mini` foundation model was retained instead of copying Mio's potentially problematic model state. Structural validation found no missing objectives, Decisions, Trigger messages, or destinations. `runtime_bootstrap` is the Start Section. The two `probe_*` Sections and `probe_start` remain as dormant backend diagnostics but are not part of the formal flow. Unity now uses kiki's formal Section IDs, has `ConvaiNarrativeBridge` enabled, and has the temporary probe Trigger/Collider disabled.

Narrative polish (2026-08-28): the Convai prototype now requires the Office return Area Trigger to observe one player exit before a later enter can send `office_exploration_returned`, preventing an immediate false return when Explore Office begins. Once Unity reaches the Memory Room or a later physical state, `ConvaiNarrativeBridge` rejects stale `office_*` Section side effects and re-sends the Trigger for Unity's authoritative stage. The second-placement observation, All Released praise, and final Distribution Confirmation use Convai speech/turn completion as a release gate with a 25-second timeout. All Released now teleports only after kiki praises all four releases and reports signal recovery; final Confirm now enters `office_report` and teleports only after kiki confirms distribution and announces the Office return. The recorded-flow scene keeps its original local-dialogue fallback when no bridge subscriber exists.

kiki's backend Objectives and Speaking Style were also tightened: `office_exploration` cannot say welcome back before the physical return Trigger; `memory_room_warning` is a one- or two-sentence private observation with no investigation prompt; `giant_crisis` treats the event as unknown, undocumented, and frightening rather than naming a Trigger or crisis; and emotional delivery now scales from curiosity through panic to relief without exposing Narrative implementation terms. Restart Play Mode after these dashboard changes so the Convai room opens a fresh session with the updated configuration.

Late-flow polish (2026-08-28): runtime logs showed Push To Talk producing final transcripts but frequently splitting utterances and completing interrupted turns. The Convai prototype still uses Push To Talk, but now sends explicit VAD values (`confidence=0.5`, `start=0.1s`, `stop=0.25s`, `min volume=0.25`) instead of server defaults, uses a 500ms release tail, and enables the SDK speech-stopped fallback. Interrupt-on-press and the turn-completion guard remain enabled; the SDK explicitly clears the wait and opens capture when interruption is allowed.

The late narrative now has distinct physical-event Sections. `swallow_started` enters `swallow_transition` as Unity enters `SwallowTransition`; `player_swallowed` enters the revised unfamiliar `mirror_chamber_introduction` only after Unity reaches `MirrorChamber`. `final_distribution_confirmed` now enters `final_distribution_confirmation` for the pre-teleport line. After the fade-in completes, Unity sends `office_returned` to `office_report`, which proactively announces that the Office return is complete and the report is ready. Pressing the report Close button invokes `report_closed` to enter `session_complete`; Unity waits for kiki's farewell turn to finish, with a 25-second timeout, before entering `SessionComplete` and fading to black.

Quest Push To Talk (2026-08-28): SDK 4.5.0 already reads Meta Quest face buttons through Unity XR input, so no project-specific controller bridge is needed. `scene_VR_ConvaiPrototype.unity` configures `ConvaiRoomManager._pushToTalkKey` as `KeyCode.JoystickButton2` (serialized value `332`), which the SDK maps to the left controller primary button, X. Hold X to capture and release X to send. The previous keyboard T binding is replaced in this prototype because the official room setting represents one control at a time. The Convai native audio layer requests Android microphone permission at runtime and fails closed if the selected controller disconnects during a press.

Conversational flexibility (2026-08-28): kiki's global Core Description and Speaking Style now use an answer-first recovery policy. Harmless general or unexpected questions receive a brief useful answer followed by at most one gentle bridge to the current task. Fragmented transcripts are paraphrased into one focused clarification question instead of a generic refusal. These global rules do not grant future-stage knowledge or permission to report unconfirmed Unity actions.

The two Memory Room Sections now carry stage-appropriate memory knowledge. `memory_room_initial_placement` explicitly defines Focus, Context, and Background, explains the water bottle, sunset photograph, and LEGO bricks, and answers proposed placement questions through neutral consequences and tradeoffs. `final_memory_placement` defines all seven memories and supports questions about item meaning or any proposed final shelf without prescribing a correct distribution. `mirror_chamber_introduction` no longer becomes unresponsive after its opening: it may answer player-initiated questions about the unfamiliar environment, objects, and shattered mirrors while remaining quiet during uninterrupted local mirror dialogue.

Unity now supplies grounded scene facts through silent tracked Dynamic Context. Every `GameStateController` state change updates the current room, current Unity stage, available memories, and current task. Every successful mirror resolution increments `MirrorMemoryResolved`, updating `Resolved mirror count` and recording that a mirror shattered because its painful memory was released rather than erased. The update uses `ConvaiRespondMode.Silent`, so it changes context without requesting an unsolicited kiki response. Unity remains the only owner of physical progression.

Post-Office Narrative integration (2026-08-27): the original bridge stopped after `memory_room_entered`; none of the later saved Triggers had Unity send points. `MemoryPlacementController` now emits `MissingPainfulMemoriesDetected` at the second distinct initial placement, and `GameStateController` exposes `AllMemoriesResolved`. `ConvaiNarrativeBridge` sends `missing_painful_memories_detected`, `crisis_started`, `player_swallowed`, `All-resolved Trigger`, `returned_to_memory_room`, and `final_distribution_confirmed` from the matching validated Unity events/state transitions, each with a once-only guard. Entering backend `session_complete` now requests the matching local close-session flow. The existing Unity gameplay remains authoritative; generated dialogue is never used directly as a game-state command.

Report-reading reliability and Editor performance (2026-08-28): multiple processed-final confirmations reached `office_report_reading`, but its natural-language Decision did not enter `office_crystal_ball`. The Decision now includes concrete positive/negative examples, and the backend includes `office_reading_finished -> office_crystal_ball`. The bridge applies the same restricted processed-final/2.5-second fallback used for Start Work and cancels it if the normal Decision arrives. Ordinary Unity `Log` stack traces are disabled in Player Settings while Warning/Error/Exception stack traces remain enabled; this reduces Editor logging stalls caused by every transcript and turn log expanding through EventHub/LiveKit. A non-development build removes Editor/XR-simulator overhead, but Convai network latency remains and Quest rendering must still be profiled separately.

The backend does not reliably report its initial Section as a Section Change. The bridge uses a dedicated `office_entered` trigger for first Office initialization and retains `office_exploration_returned` exclusively for the physical exploration return. Both target `office_introduction`, but their separate Messages and `office_entry_reason` values preserve distinct first-arrival and return greetings.

Convai delivery length is stage-aware: ordinary questions and repeat interactions use one to three short sentences, while first confirmed arrivals in the Office, initial Memory Room, and Mirror Chamber may use proactive four-to-six-sentence introductions. Exploration return remains one or two sentences and never repeats Mio's first introduction.

The prototype's world-space `TranscriptUI_Chat` uses the standard SDK 4.5.0 `ChatTranscriptUI` prefab and is parented to `AssistantRobot`. Its serialized UI references are complete. Transcript presentation was blank because `Assets/Resources/ConvaiSettings.asset` had the global Transcript System disabled; it is now enabled. The prefab intentionally begins with `CanvasGroup` alpha zero and fades in at runtime. A `KeyNotFoundException` mentioning `TranscriptUI_Chat` occurs in XR Interaction Toolkit's `TrackedDeviceGraphicRaycaster.OnDisable()` during Play Mode teardown and is not the cause of missing transcript content.
...
- `CrystalBall.cs` (VR)
  ...
  - `SetEnabled(bool)` activates/deactivates the crystal-ball GameObject (fixed 2026-08-23, matching `CrystalBall_WebGL`); the ball starts inactive in the scene and appears only when `GameStateController` enters `AwaitCrystalBall`.
...
VR scene (current development focus since 2026-08-23):

- `Assets/__My Project/scene_VR.unity` (previous iteration kept as `scene_VR_old.unity`)
